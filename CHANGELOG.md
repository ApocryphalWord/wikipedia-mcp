...
### Removed
- Removed the `wikipedia_*`-prefixed tool aliases. Every tool was previously
  registered twice (canonical name plus a `wikipedia_`-prefixed alias), which
  caused MCP clients to display each tool twice. Tools are now registered only
